...
export async function generator(state: typeof RagGraphState.State): Promise<Partial<typeof RagGraphState.State>> {
  ...
  const docsToUse = state.evaluationScore === "INCORRECT" && state.webSearchResults && state.webSearchResults.length > 0
    ? state.webSearchResults
    : state.rerankedDocuments;
  ...
  const systemPrompt = `You are Kikandai, an intelligent and highly capable assistant analyzing uploaded documents.
